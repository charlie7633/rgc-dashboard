    ...
    <div className="p-8">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm" style={{ color: "var(--text-muted)" }}>
          {products.length} products across 9 brands · {reviewed} have transcript reviews
        </p>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        {[
          ...
          { label: "Avg Unit Price", val: `£${avgPrice}`, accent: "var(--color-rgc-electric)" },
        ...
        <div className="flex gap-1.5">
          {brands.map((b) => (
            <button key={b} onClick={() => setBrandFilter(b)}
              ...
              style={{ background: brandFilter === b ? (BRAND_COLOURS[b] ?? "var(--color-rgc-electric)") : "#f1f5f9", color: brandFilter === b ? "white" : "var(--text-muted)" }}>
              {b}
            </button>
          ))}
        </div>
        ...
        {tiers.map((t) => {
          ...
          return (
            <button key={t} onClick={() => setTierFilter(t)}
              className="px-3 py-1.5 rounded-full text-xs font-medium transition-all"
              style={{ background: tierFilter === t ? (meta?.bg ?? "#f1f5f9") : "#f1f5f9", color: tierFilter === t ? (meta?.color ?? "var(--text-muted)") : "var(--text-muted)", border: tierFilter === t ? `1px solid ${meta?.color ?? "#94a3b8"}` : "1px solid transparent" }}>
              {t}
            </button>
          );
        })}
        ...
        <table className="w-full text-xs border-collapse">
          <thead>
            <tr style={{ background: "#eef2ff" }}>
              {["Product", "Brand", "Subcategory", "Price", "Pack", "Tier", "Retailers", "Reviews", "Avg Rating"].map((h) => (
                <th key={h} className="text-left px-3 py-2.5 font-semibold" style={{ color: "#4338ca" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {filtered.map((p, i) => {
              ...
              return (
                <tr key={p.productId} style={{ borderBottom: "1px solid #f1f5f9", background: i % 2 ? "#fafafa" : "white" }}>
                  ...
                  <td className="px-3 py-2.5 font-semibold" style={{ color: "var(--text-primary)" }}>£{p.price.toFixed(2)}</td>
                  <td className="px-3 py-2.5" style={{ color: "var(--text-muted)" }}>{p.packSize}</td>
                  <td className="px-3 py-2.5">
                    {tier ? (
                      <span className="px-2 py-0.5 rounded-full text-xs font-medium" style={{ background: tier.bg, color: tier.color }}>{p.priceTier}</span>
                  ...
                  <td className="px-3 py-2.5 text-center">
                    {p.reviewCount > 0 ? (
                      <span className="font-semibold" style={{ color: "var(--color-rgc-electric)" }}>{p.reviewCount}</span>
                    ) : <span style={{ color: "var(--text-muted)" }}>—</span>}
                  </td>
                  <td className="px-3 py-2.5 text-center">
                    {p.avgRating > 0 ? (
